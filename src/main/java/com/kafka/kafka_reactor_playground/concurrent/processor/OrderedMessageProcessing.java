...
public class OrderedMessageProcessing {
    ...
    public Function<List<Order>, List<Message<Object>>> deliveryProcessor(DeliveryService deliveryService) {
        ...
        return orders -> this.partitionByCustomerId(orders)
                ...
                .flatMap(Collection::stream).map(this::toMessage).toList();

    }

    private Stream<List<Order>> partitionByCustomerId(List<Order> orders) {
        return orders.stream().collect(Collectors.groupingBy(Order::customerId, Collectors.toList())).values().stream();
    ...
    private Message<Object> toMessage(Object payload) {
        ...
        return MessageBuilder.withPayload(payload).setHeader(SPRING_CLOUD_DESTINATION, destination).build();
    }
